...
from com.sun.star.io import XAsyncOutputMonitor
...
from .configuration import g_auth
...
class OutputStream(unohelper.Base,
    ...
    def waitForCompletion(self):
        print("gdrive.OutputStream.waitForCompletion() ***********************************")
    ...
    def _flush(self):
        ...
        with self.session.request(self.method, self.url, **self.kwargs) as r:
            ...
            if r.status_code == self.session.codes.ok:
                ...
                self.response.insertValue('Error', error)
...
class OAuth2OOo(NoOAuth2):
    ...
    def __call__(self, request):
        request.headers['Authorization'] = self.service.getToken('Bearer %s')
        return request
