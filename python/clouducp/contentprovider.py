...
class ContentProvider(unohelper.Base,
    ...
    def registerInstance(self, scheme, plugin, replace):
        ...
        msg = getMessage(self.ctx, __name__, 112, (scheme, plugin))
        logMessage(self.ctx, INFO, msg, 'ContentProvider', 'registerInstance()')
        return self
    def deregisterInstance(self, scheme, argument):
        msg = getMessage(self.ctx, __name__, 161, scheme)
    ...
    def createContentIdentifier(self, url):
        ...
        msg = getMessage(self.ctx, __name__, 131, url)
    ...
    def queryContent(self, identifier):
        ...
        if not identifier.isValid():
            msg = getMessage(self.ctx, __name__, 141, (url, self._error))
        ...
        msg = getMessage(self.ctx, __name__, 142, url)
    ...
    def compareContentIds(self, id1, id2):
        ...
        if id1.Id == id2.Id and id1.User.Id == id2.User.Id:
            msg = getMessage(self.ctx, __name__, 151, ids)
            compare = 0
        else:
            msg = getMessage(self.ctx, __name__, 152, ids)
            compare = -1
    ...
    def _getUser(self, uri, url):
        if uri is None:
            self._error = getMessage(self.ctx, __name__, 121, url)
            return User(self.ctx)
        if not uri.hasAuthority() or not uri.getPathSegmentCount():
            self._error = getMessage(self.ctx, __name__, 122, url)
            return User(self.ctx)
        name = self._getUserName(uri, url)
        if not name:
            self._error = getMessage(self.ctx, __name__, 123, url)
            return User(self.ctx)
